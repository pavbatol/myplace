...
Сервис `gateway` является центральной точкой входа для маршрутизации запросов к другим микросервисам.
   _Подключенные сервисы:_
    - `security`
    - `profile/geo`
    - `profile/profile`  
   Подключение остальных микросервисов будет выполнено в следующих Pull Requests.
  ...
  > **!** _Сейчас выполняется миграция сервиса Gateway на реактивный стек. После завершения перехода код, основанный на servlet API, будет удалён_</u>  
